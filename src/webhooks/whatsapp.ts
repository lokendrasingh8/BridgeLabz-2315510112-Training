  ...
  switch (intent.type) {
    ...
    case 'checkout': {
      if (session.cart.length === 0) {
        ...
        twiml.message(
          'For checkout, open the product links from search results and add to cart there. Payments via WhatsApp are not enabled in this demo.\n' +
            links.join('\n'),
        );
      }
      break;
    ...
    case 'search': {
      const products: NormalizedProduct[] = await searchProducts(intent.query);
