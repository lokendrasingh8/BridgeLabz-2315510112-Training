import { NormalizedProduct, UserSession } from '../config/types.js';
...
export function formatSearchResults(products: NormalizedProduct[]): string {
...
export function formatCart(session: UserSession): string {
  if (session.cart.length === 0) return 'Your cart is empty. Search for products to add items.';
  const lines: string[] = [];
